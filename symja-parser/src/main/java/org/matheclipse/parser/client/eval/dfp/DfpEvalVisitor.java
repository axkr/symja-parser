 ...
 * @see Dfp
 */
public class DfpEvalVisitor extends AbstractASTVisitor<Dfp> {
	...
	public static ASTNode parseNode(final int decimalDigits, String expression, boolean relaxedSyntax) {
		DfpEvalVisitor dfpEvaluator = new DfpEvalVisitor(decimalDigits, relaxedSyntax);
	...
	public DfpEvalVisitor(final int decimalDigits) {
		this(decimalDigits, null, false);
	}

	public DfpEvalVisitor(final int decimalDigits, ASTNode node, boolean relaxedSyntax) {
		super(relaxedSyntax);
	...
	public DfpEvalVisitor(final int decimalDigits, boolean relaxedSyntax) {
		...
		if (functionNode.size() > 0 && functionNode.getNode(0) instanceof SymbolNode) {
			...
			if (symbol.equals("If") || (fRelaxedSyntax && symbol.equalsIgnoreCase("if"))) {
				...
				if (obj instanceof IFieldElementFunctionNode) {
					return ((IFieldElementFunctionNode<Dfp>) obj).evaluate(this, functionNode);
				}
				if (functionNode.size() == 1) {
					if (obj instanceof IFieldElement0Function) {
						return ((IFieldElement0Function<Dfp>) obj).evaluate();
					}
				} else if (functionNode.size() == 2) {
					if (obj instanceof IFieldElement1Function) {
						return ((IFieldElement1Function<Dfp>) obj).evaluate(evaluateNode(functionNode.getNode(1)));
					}
					...
					if (obj instanceof IFieldElementInt2Function && arg2 instanceof IntegerNode) {
						return ((IFieldElementInt2Function<Dfp>) obj).evaluate(evaluateNode(functionNode.getNode(1)),
								((IntegerNode) arg2).getIntValue());
					}
					if (obj instanceof IFieldElement2Function) {
						return ((IFieldElement2Function<Dfp>) obj).evaluate(evaluateNode(functionNode.getNode(1)),
								evaluateNode(arg2));
					}
				}
		...
		if (functionNode.size() > 0 && functionNode.getNode(0) instanceof SymbolNode) {
			...
			if (functionNode.size() == 2) {
				...
				if (obj instanceof IBooleanFieldElement2Function) {
					return ((IBooleanFieldElement2Function<Dfp>) obj).evaluate(evaluateNode(functionNode.getNode(1)),
